...
const ROLES = ["AI 全栈工程师", "Python · JS/TS · Node.js", "Agent 工程实践中"];
...
const EXPERIENCE: {
...
}[] = [
	{
		...
		location: "杭州",
		points: [
			"架构与选型：搭建基于 FastAPI + React/Vue 的全栈模板，统一 H5、钉钉小程序到 Electron 桌面应用的跨端技术栈。",
			"全栈开发：CRM、运营报表、财务 OA、数据看板等模块深度参与，后端从业务编码深入到数据表设计与模块化架构重构。",
			"数据平台：整合钉钉审批流、ERP 残留数据、纸质文档等多源异构数据，搭建统一数据仓库，实现运营数据实时可视。",
			"AI 工作流：率先引入 Claude Code + MCP/Skills 工具链，将需求→编码→测试周期从天级显著压缩；完成 LLM API 集成并提出专用 Agent 后端方案。",
			"对外对接：对接院方 HIS 系统，推进眼科检查数据归档与院内眼科数据看板建设。",
	...
	},
	{
		...
		location: "杭州",
		points: [
			"两周交付低代码票据可视化工具（React + Vite + 拖拽插件 + SSR），支持模板拖拽生成与回填，中标三峡银行项目。",
			"识别旧版代码在重连机制与类型安全上的缺陷，TypeScript 全面重构河北交行智能设备通信 JS-SDK，修复长期重连缺陷，生产环境稳定运行。",
			"完成河南邮储企微对接、三峡银行传统前台业务数字化改造。",
		],
	},
];
		...
		<nav className="fixed top-0 inset-x-0 z-50 border-b border-white/5 bg-ink-950/70 backdrop-blur-md">
			<div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between font-mono text-sm">
				<a href="#top" className="text-neon-green glow-green">
					black<span className="text-white/40">@</span>ii3lack
				...
				<div className="card scanlines relative overflow-hidden">
					...
					<div className="p-6 sm:p-8 font-mono text-sm sm:text-base leading-relaxed">
						<p className="text-white/50">
							...
							<span className="text-neon-cyan">~</span> whoami
						</p>
						...
						<p className="mt-5 text-white/50">
							<span className="text-neon-green">➜</span>{" "}
							<span className="text-neon-cyan">~</span> cat role.txt
						</p>
						...
						<p className="mt-5 text-white/50">
							<span className="text-neon-green">➜</span>{" "}
							<span className="text-neon-cyan">~</span> location --current
						</p>
						<p className="mt-1 text-white/80">Hangzhou, Zhejiang · 浙江杭州</p>
						<p className="mt-5 text-white/50">
							<span className="text-neon-green">➜</span>{" "}
							<span className="text-neon-cyan">~</span> status
						</p>
						<p className="mt-1 text-white/80">
							在职 · 3 年以上全栈 ·{" "}
							<span className="text-neon-purple">Agent 工程实践中</span>
						</p>
					</div>
				</div>
...
function SectionHead({
	...
	return (
		<div className="reveal mb-10 flex items-end justify-between gap-4 border-b border-white/5 pb-4">
			...
			<p className="hidden sm:block font-mono text-xs text-white/40">
				{"// "}
				{sub}
			</p>
		</div>
				...
				<div className="reveal card p-6 sm:col-span-2">
					<p className="text-white/80 leading-relaxed">
						...
						<span className="text-neon-cyan">
							AI-native 工作流（Claude Code + MCP/Skills）
						</span>
						， 显著缩短需求到交付的周期。
					</p>
					<p className="mt-4 text-white/80 leading-relaxed">
						...
						<span className="text-neon-purple">
							业务沟通 → 架构设计 → 前后端 → 数据平台 → 上线
						</span>{" "}
						的完整流程，能把模糊需求拆解为可执行方案。持续追踪 AI/Agent
						领域，正在开发 Agent 应用开源项目。
					</p>
				</div>
...
function Row({ k, v, last }: { k: string; v: string; last?: boolean }) {
	return (
		<div
			className={`flex justify-between py-1.5 ${last ? "" : "border-b border-white/5"}`}
		>
					...
					<div key={p.name} className="reveal card p-6 flex flex-col">
						...
						<div className="mt-2 flex flex-wrap gap-1.5">
							{p.stack.map((s) => (
								<span
									key={s}
									className="font-mono text-[11px] text-neon-cyan/80"
								>
									{s}
								</span>
							))}
		...
		<footer className="border-t border-white/5 mt-10">
			<div className="max-w-5xl mx-auto px-5 py-10 flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-sm">
				<p className="text-white/40">
					<span className="text-neon-green">➜</span> built with Next.js ·
					Tailwind · ☕
				</p>
